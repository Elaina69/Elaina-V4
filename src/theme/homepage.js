...
const createWebmButtons = () => {
    ...
    volumeSlider.value = DataStore.get("audio-volume") * 100;
    volumeSlider.classList.add("volume-slider");

    muteUnmuteButton.classList.add("mute-unmute-button");
    muteUnmuteIcon.classList.add("mute-unmute-icon");
    muteUnmuteIcon.setAttribute("src", `${iconFolder}plugins-icons/${DataStore.get("mute-audio") ? 'mute' : 'audio'}.png`);
    ...
    volumeSlider.addEventListener('input', () => {
        ...
        DataStore.set("audio-volume", volumeValue);
        const audio = document.getElementById("bg-audio");
        audio.volume = volumeValue;

        if (volumeValue === 0) {
            DataStore.set("mute-audio", true);
            audio.muted = true;
            muteSetIconAudio();
    ...
    muteUnmuteButton.addEventListener('click', () => {
        const isMuted = !DataStore.get("mute-audio");
        DataStore.set("mute-audio", isMuted);
        audioMute();
        muteSetIconAudio();
