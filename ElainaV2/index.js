...
let pageChangeMutation = async (node) => {
	...
	if (pagename == "rcp-fe-lol-home-main") {
		...
		if (data["Auto-Find-Queue-(Open-Client)"] && !data["Aram-only-mode"]) {
		...
		else if (data["Auto-Find-Queue-(Open-Client)"] && data["Aram-only-mode"]) {
			await fetch('/lol-lobby/v2/lobby', {
				method: 'POST',
	...
	if (pagename == "rcp-fe-lol-postgame") {
		...
		if (data['Auto-Find-Queue-(Post-Game)'] && !data["Aram-only-mode"]) {
			await fetch('/lol-lobby/v2/lobby', {
				method: 'POST',
				body: JSON.stringify({ queueId: data["Queue-ID"] }),
				headers: {
		...
		else if (data['Auto-Find-Queue-(Post-Game)'] && data["Aram-only-mode"]) {
			await fetch('/lol-lobby/v2/lobby', {
				method: 'POST',
				body: JSON.stringify({ queueId: 450 }),
				headers: {
