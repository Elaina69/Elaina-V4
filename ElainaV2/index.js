...
if (!DataStore.has('wallpaper-index')) {
	DataStore.set('wallpaper-index', default_wallpaper-1)
}
else {}

if (!DataStore.has('audio-index')) {
	DataStore.set('audio-index', default_audio-1)
}
else {}
//___________________________________________________________________________//
...
function next_wallpaper() {
	...
	console.log("Now playing "+data["wallpaper_list"][DataStore.get('wallpaper-index')])
...
function nextSong() {
	if (data["Continues_Audio"]) {
		...
		audio_play_pause()
		console.log("Now playing "+data["audio_list"][DataStore.get('audio-index')])
	}
	else {
		songIndex++
		...
		audio_play_pause()
		console.log("Now playing "+data["audio_list"][songIndex])
	}
}
function prevSong() {
	if  (data["Continues_Audio"]) {
...
window.addEventListener('load', () => {
	...
	const audio = document.createElement("audio");
		...
		video.autoplay = true;
		video.loop     = true;
		video.src      = `//plugins/ElainaV2/assets/Backgrounds/${wallpapers[DataStore.get('wallpaper-index')]}`
    	...
    	audio.loop     = false;
		if (data["Continues_Audio"]) {
			audio.src      = `//plugins/ElainaV2/assets/Backgrounds/Audio/${Audios[DataStore.get('audio-index')]}`
		}
		else {
	...
	if (data['Continues_Audio']) {
		console.log("Now playing "+data["wallpaper_list"][DataStore.get('wallpaper-index')]+" and "+data["audio_list"][DataStore.get('audio-index')])
	}
	else {
		console.log("Now playing "+data["wallpaper_list"][DataStore.get('wallpaper-index')]+" and "+data["audio_list"][songIndex])
	}
